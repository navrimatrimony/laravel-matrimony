...
/**
 * Re-digest every stored agreement snapshot now that the online meeting fee is
 * part of the hashed package payload.
 *
 * A second pass is needed rather than an edit to the 2026_08_01_122000 backfill:
 * that one has already run on production, so amending it would be a no-op there
 * while silently diverging from what production actually executed.
 *
 * per_meeting_online_fee_amount is NULL on every row that exists today, so no
 * package fact changed — only the shape of the digest did. Left unrepaired,
 * every stored hash would stay the old, shorter one and each existing agreement
 * would read as "package changed": accepted terms would be treated as stale and
 * payment requests refused for agreements nobody touched.
 *
 * The new hash comes from the agreement engine itself for the same reason as
 * last time — a payload copied into this file would be free to drift from the
 * engine on the next change to either side, which is the exact bug class this
