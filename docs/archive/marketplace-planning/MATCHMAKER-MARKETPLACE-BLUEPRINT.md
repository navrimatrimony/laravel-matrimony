...
| D17 | **No ceiling on meetings, and no running total either.** How many people to meet is the family's own decision, and every meeting already requires their approval one at a time — so a cap protects against nothing while reading as a quota the Suchak intends to fill. The approval screen shows **this meeting's fee alone**. An earlier draft of this document showed the accumulated total (`3 भेटी · ₹9,000`) beside it, and the product owner rejected it: a family reading that line does not think "I am informed", they think *"₹9,000 वाया घालवले"* — a regret ledger presented at the exact moment they are deciding whether to hope again. The cumulative figure is still recorded and still theirs to see — it belongs on the payments screen, where a person has gone to look at money, not on the screen where they are deciding about a person (product owner's correction, 2026-08-01) |
...
| M5 | **No meeting is charged that the customer did not approve.** Approval is per meeting, and the screen carries that meeting's fee only — never the accumulated total (D17) |
...
| A9 | Unbounded meeting-fee exposure on a poor family | Closed by the flow itself — a meeting exists only if the customer approved it, individually, at a fee frozen before any of it began. The protection is the per-meeting refusal, not a displayed total (D17) |
...
| **1** | Copy the fees onto the package and into the hashed snapshot (§5.3); the engagement object (§6.1); customer-side acceptance page over the tokenised link; allow-list snapshot at proposal time; online-fee column; installments; per-meeting approval carrying that meeting's fee only (D17) | **A customer can accept a complete price agreement, and the acceptance actually freezes it.** Fold the hash change into the acceptance commit — alone it is invisible and will be deferred |
...
| A declared ceiling on chargeable meetings protects the family | It protects nothing — a meeting only exists if the family approved it, one at a time — and it reads as a quota to be filled. How many people to meet is their decision alone (D17). (Product owner's correction) |
| A running total at approval (`3 भेटी · ₹9,000`) informs the family | My replacement for the ceiling, and wrong for the same reason as the ceiling: I was still trying to protect the family with a number on a screen. What that line actually does is tell someone deciding whether to hope again that they have already wasted ₹9,000 — a regret ledger delivered at the worst possible moment. This is the second time the same instinct produced the same mistake. The approval screen carries this meeting's fee, nothing more. (Product owner's correction) |
