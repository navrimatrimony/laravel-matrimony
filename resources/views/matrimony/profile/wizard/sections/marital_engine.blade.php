...
@php
    ...
    $marriagesSuffix = ']';
    $maritalFieldClass = 'w-full min-w-0 rounded border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 px-2 py-2 text-sm h-[42px]';
@endphp
<div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-white dark:bg-gray-800 space-y-7"
    ...
    <div class="marital-details-block" data-lv-scroll-target="marital-details" x-show="statusKey === 'divorced' || statusKey === 'annulled' || statusKey === 'separated' || statusKey === 'widowed'" x-cloak style="display: none;">
        ...
        @else
            {{-- Dynamic heading by selected marital status --}}
            <div class="pb-3 mb-3 border-b border-gray-200 dark:border-gray-600">
                <h3 class="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-100" x-show="statusKey === 'divorced'" x-cloak style="display: none;">{{ __('wizard.status_section_divorced') }}</h3>
                <h3 class="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-100" x-show="statusKey === 'annulled'" x-cloak style="display: none;">{{ __('wizard.status_section_annulled') }}</h3>
                <h3 class="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-100" x-show="statusKey === 'separated'" x-cloak style="display: none;">{{ __('wizard.status_section_separated') }}</h3>
                <h3 class="text-sm sm:text-base font-semibold text-gray-800 dark:text-gray-100" x-show="statusKey === 'widowed'" x-cloak style="display: none;">{{ __('wizard.status_section_widowed') }}</h3>
            </div>
        @endif
        <div class="flex flex-col w-full min-w-0 gap-4" data-lv-section="marital-details">
        @unless ($hideStatusDetailsOptional)
        {{-- Mobile: 2×2 grid. Desktop: 4 equal columns in one row (legal spans 2 when shown). --}}
        <div class="grid w-full min-w-0 grid-cols-2 gap-2 items-end md:grid-cols-4 md:gap-2">
            <div class="min-w-0 md:col-span-1" :class="statusKey === 'widowed' ? 'md:col-span-2' : ''">
                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">{{ __('wizard.marriage_year') }}</label>
                <input type="number" name="{{ $marriagesPrefix }}marriage_year{{ $marriagesSuffix }}" min="1901" max="{{ date('Y') }}"
                       ...
                       class="{{ $maritalFieldClass }}">
            </div>
            <div class="min-w-0 md:col-span-1" x-show="statusKey === 'divorced' || statusKey === 'annulled'">
                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">
            ...
            <div class="min-w-0 md:col-span-1" x-show="statusKey === 'separated'" x-cloak>
                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">{{ __('wizard.separation_year') }}</label>
            ...
            <div class="min-w-0 md:col-span-2" x-show="statusKey === 'widowed'">
                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">{{ __('wizard.spouse_death_year') }}</label>
            ...
            <div class="min-w-0 col-span-2 md:col-span-2" x-show="statusKey === 'divorced' || statusKey === 'annulled' || statusKey === 'separated'">
                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">
                    <span x-show="statusKey === 'divorced'">{{ __('wizard.legal_status') }}</span>
                    <span x-show="statusKey === 'separated'" x-cloak style="display: none;">{{ __('wizard.legal_status_separated') }}</span>
                    <span x-show="statusKey === 'annulled'" x-cloak style="display: none;">{{ __('wizard.legal_status_annulled') }}</span>
                </label>
                <select name="{{ $marriagesPrefix }}divorce_status{{ $marriagesSuffix }}" class="{{ $maritalFieldClass }}">
                    <option value="">—</option>
        ...
        {{-- Children Yes/No --}}
        <div @class([
            'flex flex-col sm:flex-row sm:items-end gap-2 sm:gap-4',
            'pt-1 border-t border-gray-100 dark:border-gray-700/80' => ! $hideStatusDetailsOptional,
        ])>
            @unless ($hideStatusDetailsOptional)
            <h3 class="text-sm font-semibold text-gray-800 dark:text-gray-100 shrink-0 pt-3 sm:pt-0 sm:pb-2">{{ __('Children') }}</h3>
            @endunless
            <div class="min-w-0 w-full sm:w-auto sm:max-w-[11rem]">
                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 sm:sr-only">{{ __('Children') }}</label>
                <input type="hidden" name="{{ $isSnapshot ? 'snapshot[core][has_children]' : 'has_children' }}" :value="hasChildrenValue">
                <div class="inline-flex w-full rounded-full border-2 border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 p-0.5 h-[42px] items-stretch" role="group" aria-label="{{ __('Children') }}">
                    <button type="button" @click="hasChildrenValue = '0'; onHasChildrenChange()"
                            class="flex-1 px-4 py-2 text-sm font-medium rounded-full transition-all min-w-0 flex items-center justify-center"
    ...
    <div id="marital-children-details" class="scroll-mt-28" data-lv-scroll-target="children-details" tabindex="-1" x-show="showChildrenDetails" x-cloak style="display: none;">
        ...
        <div class="space-y-3" x-ref="childrenContainer">
            <template x-for="(child, index) in children" :key="'child-' + index">
                ...
                >
                    {{-- Child fields: equal width on all screen sizes — 3 columns, same height --}}
                    <div class="grid w-full min-w-0 grid-cols-3 gap-2 items-end">
                        <div class="min-w-0">
                            <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">{{ __('wizard.gender') }}</label>
                            <select :name="(namePrefix ? 'snapshot[children][' : 'children[') + index + '][gender]'" class="{{ $maritalFieldClass }}" x-model="child.gender">
                        ...
                        <div class="min-w-0">
                            <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">{{ __('wizard.age') }}</label>
                            <input type="number" :name="(namePrefix ? 'snapshot[children][' : 'children[') + index + '][age]'" min="1" max="120" class="{{ $maritalFieldClass }}" x-model.number="child.age" placeholder="{{ __('wizard.age') }}">
                        </div>
                        <div class="min-w-0">
                            <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 leading-snug">{{ __('wizard.living_with_optional') }}</label>
                            <select :name="(namePrefix ? 'snapshot[children][' : 'children[') + index + '][child_living_with_id]'" class="{{ $maritalFieldClass }}" x-model="child.child_living_with_id">
                                <option value="">{{ __('wizard.select') }}</option>
                    ...
                    <div class="flex items-center justify-between gap-3 pt-2 mt-1 border-t border-gray-200/80 dark:border-gray-600/80">
