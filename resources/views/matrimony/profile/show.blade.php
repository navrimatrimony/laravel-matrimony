...
@endphp
        ...
        @if (! ($isOwnProfile ?? false) && auth()->check())
            ...
            >
                ...
                <a
                    href="{{ route('matrimony.profile.gunamilan', $profile) }}"
                    class="inline-flex min-h-[3.25rem] w-full items-center justify-start gap-2.5 rounded-2xl border border-amber-200/90 bg-white px-3 py-2.5 text-left shadow-sm ring-1 ring-amber-100/80 transition hover:bg-amber-50/90 dark:border-amber-800/55 dark:bg-gray-800 dark:ring-amber-900/35 dark:hover:bg-amber-950/20"
                >
                    ...
                    <span class="min-w-0 flex-1">
                        <span class="block text-[11px] font-semibold uppercase tracking-wide text-amber-800/95 dark:text-amber-200/90">{{ __('profile.gunamilan_button') }}</span>
                        <span class="block truncate text-xs font-semibold text-stone-900 dark:text-stone-100">{{ __('profile.gunamilan_button_sub') }}</span>
                    </span>
                </a>
