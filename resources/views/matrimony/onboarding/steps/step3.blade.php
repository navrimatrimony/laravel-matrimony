<form method="POST" action="{{ route('matrimony.onboarding.store', ['step' => 3]) }}" class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900/20 p-4 space-y-2">
        ...
        @php
            $onbResHints = $profile->residenceLocationHierarchyHints();
        @endphp
        <x-profile.location-typeahead
            context="residence"
            mode="simple"
            :noBorder="true"
            :value="old('location_input', old('wizard_residence_display', $profile->residenceLocationDisplayLine()))"
            :placeholder="__('wizard.type_city_area')"
            :dataLocationId="$onbResHints['location_id']"
            :dataCountryId="$onbResHints['country_id']"
            :dataStateId="$onbResHints['state_id']"
            :dataDistrictId="$onbResHints['district_id']"
            :dataTalukaId="$onbResHints['taluka_id']"
