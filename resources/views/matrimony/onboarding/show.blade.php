            ...
            <header>
                ...
                @if ($step === 2)
                    <div class="mt-4 flex flex-col sm:flex-row sm:items-center gap-3">
                        <p class="text-sm text-gray-600 dark:text-gray-300 sm:flex-1">{{ __('onboarding.intake_autofill_hint') }}</p>
                        <a
                            href="{{ route('intake.upload') }}"
                            class="inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800"
                        >
                            {{ __('onboarding.intake_autofill_cta') }}
                        </a>
                    </div>
                @endif
            </header>
