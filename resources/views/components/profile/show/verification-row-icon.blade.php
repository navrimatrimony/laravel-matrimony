@props(['variant' => 'ok', 'rowKey' => ''])

@php
    $type = match (true) {
        ($rowKey === 'email') => 'email',
        ($rowKey === 'mobile') => 'mobile',
        ($rowKey === 'whatsapp') => 'whatsapp',
        ($rowKey === 'kyc') => 'id',
        str_starts_with($rowKey, 'verification_tag_') => 'tag',
        default => 'default',
    };
    $circleClass = $variant === 'ok'
        ? 'flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300'
        : 'flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-red-100 text-red-600 dark:bg-red-950/60 dark:text-red-300';
@endphp
