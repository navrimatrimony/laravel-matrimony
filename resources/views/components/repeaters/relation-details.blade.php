...
@php
    $hasOccEngine = \Illuminate\Support\Facades\Schema::hasColumn('matrimony_profiles', 'occupation_master_id');
...
@endphp
<style>
.relation-engine-row .relation-input-h {
    vertical-align: middle;
}
.relation-engine-row .occupation-engine--form-field .ts-wrapper {
    vertical-align: middle;
}
...
.relation-engine-row .relation-address-cell .location-gps-btn {
    height: 2.5rem;
    width: 2.5rem;
...
.relation-engine-row .relation-address-cell .location-typeahead-wrapper > label.block {
    font-size: 0.75rem;
    font-weight: 500;
...
<div class="space-y-4 border border-gray-200 dark:border-gray-600 rounded-lg p-4" data-relation-engine data-show-married="{{ $showMarried ? '1' : '0' }}" data-address-only-relation="{{ $addressOnlyRelationValue ?? '' }}"
    ...
    @foreach($rows as $idx => $row)
        ...
        <div class="{{ $namePrefix }}-row relation-engine-row mb-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600 space-y-3">
            ...
            @if($showMarried)
            {{-- Line 1: 12-col grid — Relation 3 | Name 4 | Married 2 | Mobile 3 --}}
            <div class="grid grid-cols-12 gap-3 items-center">
                ...
                <div class="col-span-12 sm:col-span-3 min-w-0" data-contact-context="sibling" data-row-index="{{ $idx }}" data-name-prefix="{{ $namePrefix }}">
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-0.5">{{ __('components.relation.mobile') }}</label>
                    <div class="flex flex-nowrap items-center gap-1 sibling-contact-slots-inner min-w-0">
                        @php
            ...
            @if($showMarried)
            {{-- Line 2: Occupation 4 | Address 4 | Additional 4 --}}
            <div class="grid grid-cols-12 gap-3 items-center">
                <div class="col-span-12 sm:col-span-4 min-w-0">
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-0.5">{{ __('components.relation.occupation') }}</label>
                    @if ($hasOccEngine)
                ...
                <div class="col-span-12 sm:col-span-4 min-w-0 relation-address-cell">
...
<template id="sibling-contact-slot-tpl">
    ...
    <div class="contact-field-engine contact-field-engine--inline">
        <div class="flex items-center gap-1.5 flex-nowrap contact-master-field">
            <input type="text" inputmode="tel" maxlength="5" value="+91" placeholder="+91" class="text-xs text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-gray-600 rounded py-1.5 bg-gray-50 dark:bg-gray-700 h-10 box-border text-center shrink-0 contact-cc-input" style="flex:0 0 2.25rem; width:2.25rem; min-width:2.25rem; max-width:2.25rem; padding-left:0.2rem; padding-right:0.2rem;">
            <input type="text" inputmode="numeric" pattern="[0-9]*" maxlength="10" name="__NAME__" placeholder="{{ __('components.relation.ten_digit') }}" data-contact-engine class="relation-input-h h-10 rounded border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 px-2 py-1.5 text-sm min-w-0 flex-1">
            ...
            <div class="relative shrink-0 contact-preference-single" data-current-pref="call">
                <button type="button" class="contact-pref-trigger inline-flex items-center justify-center h-10 w-10 shrink-0 rounded p-1.5 ring-1 ring-gray-300 dark:ring-gray-600 bg-gray-50 dark:bg-gray-700/50" title="{{ __('contact.prefer_contact_via') }}" aria-haspopup="true" aria-expanded="false">
            ...
            <button type="button" class="contact-engine-add-btn shrink-0 inline-flex items-center justify-center w-10 h-10 rounded border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 font-bold text-lg leading-none hover:bg-gray-100 dark:hover:bg-gray-600/50" title="{{ __('contact.add_another_contact') }}" aria-label="{{ __('contact.add_contact') }}">+</button>
        </div>
    </div>
    </div>
</template>
