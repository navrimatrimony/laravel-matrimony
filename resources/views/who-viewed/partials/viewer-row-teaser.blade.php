...
@endphp
<div class="flex w-full flex-col items-stretch overflow-hidden rounded-xl border border-indigo-200/80 bg-gradient-to-br from-white via-indigo-50/40 to-indigo-100/50 shadow-sm ring-1 ring-indigo-100/60 dark:border-indigo-800/70 dark:from-gray-900 dark:via-indigo-950/30 dark:to-indigo-950/50 dark:ring-indigo-900/40 sm:min-h-[10rem] sm:flex-row">
    {{-- Photo: stretch with row height; blur optional --}}
    <div class="relative h-44 w-full shrink-0 overflow-hidden bg-gray-900 sm:h-auto sm:w-32 sm:self-stretch">
        @if ($photoUrl)
        ...
        <div class="pointer-events-none absolute inset-0 z-[1] flex flex-col items-center justify-center gap-2 bg-gradient-to-b from-black/10 via-black/25 to-black/45 px-2 py-2 sm:gap-3">
            ...
            <p class="max-w-[90%] text-center text-[10px] font-semibold leading-snug text-white/90 sm:text-[11px]">
                {{ __('who_viewed.teaser_photo_paid_hint') }}
            </p>
        </div>
    </div>

    {{-- Details: natural height, no forced inner scroll --}}
    <div class="flex min-h-0 min-w-0 flex-1 flex-col justify-start border-t border-indigo-200/50 bg-white/40 px-3 py-2.5 dark:border-indigo-800/50 dark:bg-gray-900/20 sm:border-l sm:border-t-0">
        <p class="font-semibold text-gray-900 dark:text-gray-100">{{ $teaser['headline'] ?? '' }}</p>
        @if ($accentLine)
            <p class="mt-1.5 text-sm font-semibold text-amber-700 dark:text-amber-300">{{ $accentLine }}</p>
        @endif
        @if ($matchLine)
            <p class="mt-1 text-sm font-semibold text-emerald-700 dark:text-emerald-400">{{ $matchLine }}</p>
        @endif
        ...
        <p class="mt-2 shrink-0 break-words text-xs text-gray-500 dark:text-gray-400">{{ $teaser['viewed_summary'] ?? '' }}</p>
        @if ($hideRightCta)
    ...
    @if (! $hideRightCta)
        {{-- CTA column --}}
        <div class="flex w-full shrink-0 flex-col items-stretch justify-center gap-2.5 border-t border-indigo-200/60 bg-gradient-to-b from-indigo-600/95 via-indigo-700 to-indigo-800 px-3 py-3 text-white shadow-inner dark:border-indigo-700/80 sm:h-full sm:w-36 sm:border-l sm:border-t-0 sm:px-2.5 sm:py-2.5">
            <div class="flex flex-col items-center gap-1">
                <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/10 ring-1 ring-white/25">
                    <svg class="h-5 w-5 text-amber-100" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 00-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                    </svg>
                </span>
                <p class="text-center text-[10px] font-medium leading-snug text-indigo-100/95 sm:text-[10px]">
                    {{ $teaser['interest_hint'] ?? __('who_viewed.teaser_cta_column_hint') }}
                </p>
            </div>
            <a href="{{ $plansUrl }}" title="{{ __('who_viewed.teaser_unlock_cta') }}" class="group inline-flex w-full items-center justify-center gap-1.5 rounded-xl bg-white px-2 py-2 text-center text-[13px] font-bold text-indigo-800 shadow-lg ring-2 ring-white/40 transition hover:bg-amber-50 hover:text-indigo-900 hover:ring-amber-200/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 focus-visible:ring-offset-2 focus-visible:ring-offset-indigo-700 sm:text-sm">
