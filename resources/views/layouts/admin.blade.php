    ...
    <div class="min-h-screen flex">
        <aside class="admin-sidebar w-64 flex-shrink-0 bg-gray-800 text-gray-300 fixed inset-y-0 left-0 z-30 flex flex-col min-h-screen h-screen" style="width: 16rem;">
            ...
            <nav class="p-4 flex-1 overflow-y-auto overscroll-contain space-y-2">
                @php
                    $moderationOpen = request()->routeIs('admin.abuse-reports.*');
                    $settingsOpen = request()->routeIs('admin.demo-search-settings.*') || request()->routeIs('admin.view-back-settings.*') || request()->routeIs('admin.notifications.*') || request()->routeIs('admin.profile-field-config.*');
                @endphp
