...
<div class="max-w-2xl bg-white dark:bg-gray-800 shadow rounded-lg p-6">
    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">Restore Serious Intent</h1>
    ...
    <div class="mb-6 p-4 border rounded bg-gray-50 dark:bg-gray-700/40">
        <p class="text-sm text-gray-600 dark:text-gray-300">Intent name</p>
        <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $intent->name }}</p>
    </div>

    <form method="POST" action="{{ route('admin.serious-intents.restore', $intent->id) }}">
        @csrf
        ...
        <div class="mt-4 flex gap-3">
            ...
            <a href="{{ route('admin.serious-intents.index') }}" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-md">Cancel</a>
        </div>
    </form>
</div>
@endsection
