...
<div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
    <div class="p-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">Add Caste</h1>
        <form method="POST" action="{{ route('admin.master.castes.store') }}" class="max-w-md space-y-4">
            @csrf
            <div>
                ...
                <select name="religion_id" id="religion_id" required
                    ...
                    @foreach ($religions as $r)
                        <option value="{{ $r->id }}" {{ old('religion_id') == $r->id ? 'selected' : '' }}>{{ $r->label }}</option>
                    @endforeach
                </select>
                @error('religion_id')
