...
<div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">Verification Tags</h1>
    ...
    <div class="mb-8 p-4 border rounded-lg bg-gray-50 dark:bg-gray-700/40">
        <h2 class="text-lg font-semibold mb-3 text-gray-800 dark:text-gray-100">Create New Tag</h2>
        <form method="POST" action="{{ route('admin.verification-tags.store') }}" class="flex gap-3 items-end">
            @csrf
            <div class="flex-1">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full border border-gray-200 dark:border-gray-700">
            <thead class="bg-gray-100 dark:bg-gray-700">
                <tr>
                    <th class="text-left px-4 py-2 border-b">Tag</th>
            ...
            <tbody>
                @forelse ($tags as $tag)
                    <tr class="border-b border-gray-200 dark:border-gray-700">
                        <td class="px-4 py-3">
                            @if ($tag->trashed())
                                <span class="text-gray-500 line-through">{{ $tag->name }}</span>
                            @else
                                <form method="POST" action="{{ route('admin.verification-tags.update', $tag->id) }}" class="flex gap-2 items-center">
                                    @csrf
                                    @method('PUT')
                                    <input
                                        type="text"
                        ...
                        <td class="px-4 py-3">
                            @if ($tag->trashed())
                                <a href="{{ route('admin.verification-tags.restore-confirm', $tag->id) }}"
                                   class="inline-block px-3 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded">
                                    Restore
                                </a>
                            @else
                                <form method="POST" action="{{ route('admin.verification-tags.destroy', $tag->id) }}" class="flex gap-2 items-center">
                                    @csrf
                                    @method('DELETE')
                                    <input
                                        type="text"
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-4 py-4 text-gray-500">No tags found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
