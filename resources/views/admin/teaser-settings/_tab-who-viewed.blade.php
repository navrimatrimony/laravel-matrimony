<form method="POST" action="{{ route('admin.teaser-settings.who-viewed.update') }}" class="space-y-6">
    @csrf
    ...
    <div class="rounded-lg border border-gray-200 bg-gray-50/80 p-4 dark:border-gray-600 dark:bg-gray-900/40">
        <label class="flex items-start gap-2 text-sm text-gray-800 dark:text-gray-100">
            <input type="checkbox" name="apply_who_viewed_locked" value="1" @checked(! empty($policy['apply_who_viewed_locked'] ?? true)) class="mt-0.5 rounded border-gray-300 dark:border-gray-600">
            <span><strong>Apply</strong> the rules below to locked / hidden “who viewed me” rows (when the member does not have full access).</span>
        </label>
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">If unchecked, those rows use a minimal anonymous card (silhouette, no extra lines). Stored in <code class="text-xs">who_viewed_teaser_policy_json</code>.</p>
    </div>
    ...
    <div>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Uses canonical <code class="text-xs">location_id</code> hierarchy only.</p>
    </div>
    ...
    <div>
        ...
        <select name="name_display" class="w-full max-w-md rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm shadow-sm">
            ...
            <option value="masked" @selected(($policy['name_display'] ?? 'masked') === 'masked')>Masked (e.g. A•••••)</option>
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Masked name — number of dots (after first letter)</label>
    ...
    <div class="space-y-3">
        ...
        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
            <input type="checkbox" name="show_marital_status" value="1" @checked(! empty($policy['show_marital_status'])) class="rounded border-gray-300 dark:border-gray-600"> Marital status label
        </label>
        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
            <input type="checkbox" name="show_repeat_view_teaser" value="1" @checked(! empty($policy['show_repeat_view_teaser'] ?? true)) class="rounded border-gray-300 dark:border-gray-600"> “Viewed your profile N times” (when N &gt; 1)
        </label>
        <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Minimum match score (%) to show compatibility line</label>
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">{{ __('admin.who_viewed_partial_list_order_label') }}</label>
        <select name="partial_plan_list_order" class="w-full max-w-md rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm shadow-sm">
            <option value="fifo_unlocked_first" @selected(($policy['partial_plan_list_order'] ?? 'fifo_unlocked_first') === 'fifo_unlocked_first')>{{ __('admin.who_viewed_partial_list_order_fifo') }}</option>
            <option value="recent_activity_first" @selected(($policy['partial_plan_list_order'] ?? '') === 'recent_activity_first')>{{ __('admin.who_viewed_partial_list_order_recent') }}</option>
        </select>
    </div>

    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">{{ __('admin.who_viewed_per_page_label') }}</label>
        <input type="number" name="who_viewed_per_page" min="5" max="50" value="{{ old('who_viewed_per_page', (int) ($policy['who_viewed_per_page'] ?? 15)) }}" class="w-24 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm shadow-sm">
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('admin.who_viewed_per_page_help') }}</p>
    </div>

    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Max teaser cards when “who viewed” is fully locked (upgrade wall)</label>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Shown before login / paywall teaser list only (1–60).</p>
    </div>
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Blur strength (when using blurred photo)</label>
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">“Viewed …” time line</label>
