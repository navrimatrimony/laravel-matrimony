            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-200 dark:border-gray-600" x-data="{ profileLockBlurStrengthUi: {{ (int) old('profile_view_lock_blur_strength', $profileViewLockBlurStrength ?? 78) }} }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs mb-1">Locked sections — how to show (daily view limit)</label>
                        <select name="profile_view_lock_display_mode" class="block w-full max-w-xl rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm text-gray-900 dark:text-gray-100">
                            <option value="merged_blur_card" @selected(old('profile_view_lock_display_mode', $profileViewLockDisplayMode ?? 'merged_blur_card') === 'merged_blur_card')>Single blurred preview card (default)</option>
                            <option value="heading_lock_rows" @selected(old('profile_view_lock_display_mode', $profileViewLockDisplayMode ?? 'merged_blur_card') === 'heading_lock_rows')>Each locked section — title + lock line only (no blurred body)</option>
                            <option value="per_section_blur" @selected(old('profile_view_lock_display_mode', $profileViewLockDisplayMode ?? 'merged_blur_card') === 'per_section_blur')>Each locked section — its own small blur preview</option>
                        </select>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            Contact + WhatsApp mediator strip always stay readable above “How does your profile match…”, regardless of this choice.
                        </p>
                    </div>
                </div>
            ...
            <fieldset @disabled(!($canManageSiteIdentitySettings ?? false)) class="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-200 dark:border-gray-600 space-y-5 disabled:opacity-60">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="rounded-lg border border-dashed border-indigo-200 bg-indigo-50/50 p-3 dark:border-indigo-800 dark:bg-indigo-950/20 md:col-span-2">
                        ...
                        <p class="mt-1 text-xs text-gray-600 dark:text-gray-400">Couple photos, slider timing, and layout are managed in one place — not here.</p>
                        <a href="{{ route('admin.homepage-settings.index', ['tab' => 'stories']) }}" class="mt-2 inline-flex text-xs font-semibold text-indigo-700 hover:underline dark:text-indigo-300">Open Homepage settings → Success stories</a>
                    </div>
                </div>
            </fieldset>
        </div>
