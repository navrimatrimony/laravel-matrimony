...
/**
 * Benchmark-only EasyOCR client (HTTP sidecar or CLI fallback).
 */
class OcrEnsembleBenchmarkEasyOcrClient
{
    public const ENGINE = 'easyocr_v1';
    ...
    public function extractFromImagePath(string $absoluteImagePath): array
    {
        if (! is_file($absoluteImagePath) || ! is_readable($absoluteImagePath)) {
            throw new RuntimeException("EasyOCR input image not readable: {$absoluteImagePath}");
        }
    ...
    private function extractViaHttp(string $baseUrl, string $absoluteImagePath): array
    {
        ...
        try {
            ...
            throw new RuntimeException('EasyOCR sidecar connection failed: '.$e->getMessage(), previous: $e);
        }

        if (! $response->successful()) {
            throw new RuntimeException('EasyOCR sidecar failed: HTTP '.$response->status().' '.$response->body());
        }

        $payload = $response->json();
        if (! is_array($payload)) {
            throw new RuntimeException('EasyOCR sidecar returned invalid JSON.');
        }
    ...
    private function extractViaCli(string $absoluteImagePath): array
    {
        ...
        if ($runner === null) {
            throw new RuntimeException('EasyOCR sidecar URL is empty and CLI runner was not found.');
        }

        $python = trim((string) config('ocr.ensemble.phase2.benchmark.easyocr.python_binary', 'python3'));
        $timeout = max(5, (int) config('ocr.ensemble.phase2.benchmark.timeout_seconds', 180));
        $command = sprintf(
            '%s %s --image %s',
        ...
        if (! is_resource($process)) {
            throw new RuntimeException('Could not start EasyOCR CLI runner.');
        }
        ...
        while (true) {
            ...
            if ((microtime(true) - $started) > $timeout) {
                ...
                throw new RuntimeException('EasyOCR CLI runner timed out after '.$timeout.'s.');
            }
            usleep(100_000);
        }
        ...
        if ($exitCode !== 0) {
            throw new RuntimeException('EasyOCR CLI runner failed: '.trim($stderr !== '' ? $stderr : $stdout));
        }

        $payload = json_decode(trim($stdout), true);
        if (! is_array($payload)) {
            throw new RuntimeException('EasyOCR CLI runner returned invalid JSON.');
        }
    ...
    {
        return rtrim(trim((string) config('ocr.ensemble.phase2.benchmark.easyocr.sidecar_url', '')), '/');
    }

    private function cliRunnerPath(): ?string
    {
        $configured = trim((string) config('ocr.ensemble.phase2.benchmark.easyocr.cli_runner', ''));
        if ($configured !== '' && is_file($configured)) {
            return $configured;
        }

        $default = base_path('tools/ocr-ensemble-easyocr-sidecar/run_ocr.py');
