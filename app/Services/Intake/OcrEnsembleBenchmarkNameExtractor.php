...
/**
 * Benchmark-only candidate name extraction and cleanup from OCR text.
 */
class OcrEnsembleBenchmarkNameExtractor
{
    /**
    ...
    public function extract(array $lines, ?string $coreName = null, ?string $hintName = null): ?string
    {
        ...
        foreach ([$coreName, $hintName] as $candidate) {
            $cleaned = $this->cleanBenchmarkName($candidate);
        ...
        foreach ($lines as $index => $line) {
            ...
            if ($this->hasCandidateNameLabel($line)) {
                ...
                $cleaned = $this->cleanBenchmarkName($value);
                if ($this->validCandidateName($cleaned)) {
        ...
        foreach ($this->candidateScopedLines($lines) as $line) {
            ...
            $cleaned = $this->cleanBenchmarkName($line);
    ...
    public function cleanBenchmarkName(?string $name): ?string
    {
