        ...
        DB::transaction(function () {
            ...
            foreach ($profiles as $profile) {
                ...
                if ($profile->religion) {
                    ...
                    $religion = DB::table('master_religions')
                        ->where('key', $religionKey)
                        ->first();

                    if (! $religion) {
                        $religionId = DB::table('master_religions')->insertGetId([
                            'key' => $religionKey,
                ...
                if ($profile->caste) {
                    ...
                    if (! $caste) {
                        $casteId = DB::table('master_castes')->insertGetId([
                            'religion_id' => null,
                            'key' => $casteKey,
                ...
                if ($profile->sub_caste && $updatedProfile->caste_id) {
                    ...
                    $subCaste = DB::table('master_sub_castes')
                        ->where('caste_id', $updatedProfile->caste_id)
                        ->where('key', $subKey)
                        ->first();

                    if (! $subCaste) {
                        $subId = DB::table('master_sub_castes')->insertGetId([
